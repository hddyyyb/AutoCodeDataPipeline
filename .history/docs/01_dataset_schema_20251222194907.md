...
This document defines the unified dataset schema used in AutoCodeDataPipeline for generating training data from a local code repository.
...
All samples are generated automatically and include explicit code grounding and auditable reasoning traces.
...
```
The trace is not intended to expose model chain-of-thought, but to provide human-readable auditability.
...
This structure enables consistent downstream processing and evaluation.
...
New task types or additional metadata fields can be introduced without breaking existing samples.
...
The `domain_map.json` file is an intermediate artifact generated during the pipeline execution. It captures structured knowledge extracted from the repository and serves as input for downstream QA and Design generation.

### A.1 Repository Metadata
