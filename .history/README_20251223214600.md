...
## Alignment with Homework Requirements

This project is designed to explicitly address the requirements of the interview assignment
“Local Code Repository–Driven Training Data Generation and Processing”.

The alignment is summarized as follows:

### Scenario 1: Repository-Grounded QA Generation

- The system automatically extracts **business rules** and **business flows** from the local code repository.
- For each QA sample:
  - The answer is grounded in **original source code snippets**.
  - A structured, auditable **reasoning trace (trace_digest)** is generated.
- The QA task focuses on explaining:
  - How business rules are implemented
  - How key processes (e.g., order creation, stock locking) are realized in code

This directly satisfies the requirement of generating QA pairs with original code context and reasoning.

### Scenario 2: Repository-Constrained Design Generation

- Given a functional or non-functional requirement, the system generates a **design proposal constrained by the existing repository architecture**.
- Design outputs are aligned with:
  - Existing Controller / Service / Mapper layering
  - Existing domain boundaries and entities
- Each design sample includes:
  - Referenced evidence code snippets
  - A multi-step reasoning trace explaining design decisions

This ensures the design task is not speculative, but repository-grounded and auditable.

### Dataset Structure and Metadata

All generated samples follow a unified dataset schema, including:
- Task type (QA / Design)
- Language
- Domain and difficulty
- Evidence references (`chunk_id`, file path, line range)
- Original code snippets
- Structured reasoning trace

This guarantees traceability and training–inference consistency.

### Diversity and Representativeness

Dataset diversity and representativeness are ensured through:
- Multi-domain coverage (order / stock / mixed)
- Multiple task types (rule QA, flow QA, architecture design)
- Strategy-level variation in design samples
- Quota-based sampling to prevent over-representation of specific files or code chunks

Statistical summaries of dataset coverage are provided to validate these properties.
