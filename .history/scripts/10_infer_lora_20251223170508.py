...
def hard_check_output(output: str, allowed_chunks: List[Dict[str, Any]]) -> Tuple[bool, str]:
    ...
    found_fps = set(re.findall(r"([\w\-/\\.]+?\.(?:java|xml|yml|yaml|properties|py|ts|js|sql))", output))
    ...
    if bad_cids or bad_fps:
        ...
        if bad_fps:
            msg += f"\n- 非法file_path:{bad_fps}"
        msg += "\n请重试或提高检索证据覆盖(例如加入关键方法本体或mapperSQL片段)。"
        return False, msg
...
def hard_check_content(output: str, lang: str) -> Tuple[bool, str]:
    ...
    if lang == "en":
        must_have_patterns.append(r"stock|lock|reserve|deduct|release|pms_sku_stock|lock_stock|hasStock|lockStock")
    else:
        must_have_patterns.append(r"库存|锁定|stock|lock_stock|pms_sku_stock|hasStock|lockStock")
...
def repair_output(output: str, allowed_chunks: List[Dict[str, Any]], lang: str) -> str:
    ...
    # 检查推理段落步数，不足补齐
    tail_pat = r"【推理过程】([\s\S]*)$" if lang != "en" else r"\[Reasoning Trace\]([\s\S]*)$"
    m = re.search(tail_pat, output)
    if m:
        tail = m.group(1)
        ...
        if len(steps) < 3 and allowed_cids:
            ...
            if lang == "en":
                if len(steps) < 1:
                    addon.append(f"1. Locate the stock lock/reserve entry or SQL update and identify target table/fields.(chunk_id={c1})")
                if len(steps) < 2:
                    addon.append(f"2. Explain key conditions(AND/if) that ensure concurrency safety and prevent oversell.(chunk_id={c2})")
                if len(steps) < 3:
                    addon.append(f"3. Describe failure branches, rollback/release conditions, and returned status.(chunk_id={c3})")
            else:
                if len(steps) < 1:
                    addon.append(f"1. 定位库存锁定相关入口或SQL更新语句，确认更新表与字段。(chunk_id={c1})")
                if len(steps) < 2:
...
def _build_prompt(question: str, evidence_text: str, whitelist_text: str, chunks: List[Dict[str, Any]], lang: str) -> str:
    cite_lines = []
    for c in chunks[:2]:
...
You are a repository analysis assistant. Answer strictly based on the provided evidence. Do NOT fabricate APIs/return values/strings.

[Hard Constraints]
- No placeholders: xxxx, Lx, Ly, etc.
- Citations must be exactly: (chunk_id=hex) and chunk_id must come from the whitelist.
...
def main():
    ...
    for idx, (q, k) in enumerate(attempts, start=1):
        ...
        prompt = _build_prompt(question, evidence_text, whitelist_text, top_chunks, infer_lang)
    ...
    print("\n=== FINAL FAILED ===")
    print("证据不足：两轮检索仍未覆盖到明确入口/SQL更新/关键条件。")
    if last_chunks:
        print("最后一轮白名单如下(请检查是否真正包含关键方法或mapperSQL)：")
