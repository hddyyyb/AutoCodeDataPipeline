...
修复点:
- 英文样本不再“Rule <id> +统一废话”:缺少title_en/description_en时，从证据代码推断英文主题
- rule_question/flow_question支持字符串或列表，列表则随机选，提高问法多样性
- 配置文件(yaml/yml/properties)走专用英文问法，避免“conditions/branches”不贴切
- 排除模板化/低价值文件(可配置)
- 同一rule_id限流+去重(避免刷屏)
...
def infer_en_topic_from_evidence(ev: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if ext in ("yaml", "properties"):
        ...
        desc = f"Explain what this configuration controls and what the key settings mean."
        if k:
            title = f"Configuration for {k}"
            desc = f"Explain the purpose of '{k}' and how its value affects runtime behavior."
        return title, desc

    # 2) class/module role based
    role = "implementation"
    ...
    for ln in code.splitlines():
        ...
        if mm:
            ...
            break
    if not method:
        # try from key lines like "public int updateStatus(..."
        for ln in key_lines:
...
def build_flow_answer(flow: Dict[str, Any], evs: List[Dict[str, Any]], templates: Dict[str, Any], lang: str) -> str:
    ...
    for i, s in enumerate(steps, 1):
        ...
        if lang == "en" and has_cjk(step_name):
            step_name = s.get("op") or f"step{i}"
...
def generate_rule_qa(
    ...
    if lang == "en":
        ...
        # 配置文件用专用问法(覆盖templates的“branches”措辞)
    ...
    meta_v2 = {
        ...
        "generator": "step04_diverse_v3_en_infer",
