...
def build_flow_answer(flow: Dict[str, Any], evs: List[Dict[str, Any]], templates: Dict[str, Any], lang: str) -> str:
    ...
    for i, s in enumerate(steps, 1):
        ...
        if lang == "en":
            step_name = en_fallback(step_name, op or f"step{i}")
...
def generate_rule_qa(rule: Dict[str, Any], index_map: Dict[str, Dict[str, Any]], templates: Dict[str, Any],
    ...
    strict_en = os.environ.get("QA_STRICT_EN", "1").strip() == "1"

    if lang == "en":
        # 如果你后面给rules补了title_en/description_en，这里会自动用上
        title_fb = rule.get("title_en") or (f"Rule {rule_id}" if rule_id else "Rule")
        desc_fb = rule.get("description_en") or "Please explain the rule and its implementation based on the evidence code."

        # 严格模式：只要原始字段含中文就跳过，保证英文集干净
        if strict_en and (has_cjk(title_raw) or has_cjk(desc_raw)):
            return {}

        title = en_fallback(title_raw, title_fb)
        desc = en_fallback(desc_raw, desc_fb)
    else:
        title = title_raw
        desc = desc_raw
...
def generate_flow_qa(flow: Dict[str, Any], index_map: Dict[str, Dict[str, Any]], templates: Dict[str, Any],
    ...
    if lang == "en":
        flow_name_fb = flow.get("name_en") or (f"Flow {flow_id}" if flow_id else "Business flow")

        if strict_en and has_cjk(flow_name_raw):
            return {}

        flow_name = en_fallback(flow_name_raw, flow_name_fb)

        # 如果step.name含中文也跳过(避免英文样本混中文步骤名)
        if strict_en:
            for s in flow.get("steps", []) or []:
                if has_cjk(s.get("name") or ""):
                    return {}
    else:
