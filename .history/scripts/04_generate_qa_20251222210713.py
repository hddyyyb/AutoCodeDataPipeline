...
def resolve_language(default_lang: str = "zh") -> str:
    ...
    if cfg_path.exists():
        try:
            ...
            lang = cfg.get("language", {}).get("mode")
            if lang:
...
def generate_rule_qa_without_language(rule: Dict[str, Any], index_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    cid = rule["evidence_chunks"][0]
    ev = build_evidence(cid, index_map)

    #q = f"{rule['title']}在代码中是如何实现的？"
    q = f"{rule['title']}在{ev['file_path']}的相关逻辑中是如何实现的？请结合代码说明关键判断与处理分支。"
...
def generate_flow_qa_without_language(flow: Dict[str, Any], index_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    evs = []
    for s in flow["steps"]:
        cid = s.get("evidence_chunk")
        if cid and cid in index_map:
    ...
    q = f"请说明{flow['name']}流程在系统中的执行步骤，并指出每一步对应的模块/代码位置。"
...
def main():
    ...
    lang_mode = normalize_lang(raw_lang)     # 归一化为 zh/en/bilingual
