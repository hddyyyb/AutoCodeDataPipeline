...
场景2:需求->基于本地代码仓架构生成设计方案(多样性+代表性版)

特性:
- 多样性:
  - 每条随机策略组合(outbox/saga/state_machine/idempotency_key/ttl_reserve/optimistic_lock)
  - 从证据代码抽取类名/方法名注入design_output,让方案贴合不同证据
  - Instruction与trace多模板随机
- 代表性:
  - 设计样本按domain配额(stock/order/mixed)+strategy覆盖配额抽样
  - 对file_path/chunk_id限流,避免同一证据刷屏
- 输出: data/dataset/design_{train,dev,test}.jsonl + data/samples/design_samples.jsonl
"""
...
# -----------------------------
STRATEGIES = [
    {"id": "outbox", "zh": "Outbox事件表+异步投递(最终一致性)", "en": "Outbox table + async dispatcher(eventual consistency)", "points": ["outbox_event表", "投递任务", "重试与幂等消费"]},
    {"id": "saga", "zh": "Saga补偿事务(失败可逆)", "en": "Saga with compensations(failure reversible)", "points": ["补偿接口", "状态记录", "可重入执行"]},
    {"id": "idempotency_key", "zh": "幂等键(requestId)+去重表", "en": "Idempotency key(requestId)+dedup table", "points": ["dedup表", "唯一约束", "重复无副作用"]},
    {"id": "state_machine", "zh": "显式状态机校验(禁止非法流转)", "en": "Explicit state machine validation(block illegal transitions)", "points": ["allowedTransitions", "统一校验入口", "错误语义统一"]},
    {"id": "ttl_reserve", "zh": "库存预占TTL+超时释放任务", "en": "Stock reserve TTL + timeout release job", "points": ["reserve记录", "expire_at", "release job扫描"]},
    {"id": "optimistic_lock", "zh": "乐观锁/条件更新(并发安全)", "en": "Optimistic lock/conditional update(concurrency-safe)", "points": ["version字段", "where条件更新", "失败重试策略"]},
]
...
def expand_requirements_from_flows_and_rules(flows: List[Dict[str, Any]], rules: List[Dict[str, Any]], max_n: int) -> List[Dict[str, Any]]:
    ...
    for f in flows:
        ...
        name_en = (f.get("name_en") or "").strip() or name_zh
        reqs.append({
    ...
    for r in rules:
        ...
        title_en = (r.get("title_en") or "").strip() or title_zh
        reqs.append({
...
# -----------------------------
# Design generation(dynamic)
# -----------------------------
def generate_design_output(requirement: str, domain: str, repo_context: Dict[str, Any], name_hints: Dict[str, Any], strategies: List[Dict[str, Any]], lang: str) -> Dict[str, Any]:
    ...
    evidence_hints = []
    if classes:
        evidence_hints.append(f"复用/改造类:{','.join(classes[:3])}" if lang == "zh" else f"Reuse/extend classes:{', '.join(classes[:3])}")
    if methods:
        evidence_hints.append(f"关注入口方法:{','.join(methods[:4])}" if lang == "zh" else f"Focus on entry methods:{', '.join(methods[:4])}")
    if files:
        evidence_hints.append(f"证据文件:{','.join(files[:2])}" if lang == "zh" else f"Evidence files:{', '.join(files[:2])}")
    ...
    for sid in strat_ids:
        if sid == "outbox":
            ...
            components.append("StateMachineConfig(新增):allowedTransitions配置+统一校验入口")
        elif sid == "ttl_reserve":
            components.append("ReserveTTLPolicy(新增):TTL策略与过期处理")
        elif sid == "optimistic_lock":
            components.append("VersionedUpdateHelper(新增):条件更新/失败重试")
    ...
    if lang == "zh":
        ...
        overview = f"Follow existing Controller/Service/Mapper layering and apply strategy set({'; '.join(strat_text)}) with minimal invasive changes for idempotency, concurrency safety, and eventual consistency."
        sequence = [
    ...
    if lang == "en":
        variants = [
            [
                ...
                f"Step2:Inspect evidence chunks {ref_chunks[:3]} and extract reusable class/method hints",
                f"Step3:Select strategy set {strategy_ids} and map them into layered components",
                "Step4:Define APIs/data model and discuss failure modes/idempotency/consistency",
            ],
            [
                f"Step1:Set architectural constraints(Controller/Service/Mapper), domain={domain}",
                f"Step2:Ground the design on evidence chunks {ref_chunks[:2]}",
...
def build_training_text(requirement: str, design_output: Dict[str, Any], evidence_snippets: List[Dict[str, Any]], trace_steps: List[str], lang: str, context_en: str = "") -> str:
    if lang == "en":
        ins_pool = [
            ...
            "Based on the repo architecture, design a solution and provide evidence snippets plus a trace of reasoning.",
            "Design an implementation plan aligned with layered architecture and attach evidence snippets and trace.",
        ]
...
def quota_sample_design(
    ...
    # pass1:strict满足domain+strategy(策略按命中其中任意一个计数)
    for s in pool:
...
def main():
    ...
    # 代表性控制参数
    train_target = int(os.environ.get("DESIGN_TRAIN_N", "0"))  # 0表示按比例自动推
    ...
    for r in all_reqs:
        ...
        # 每个req生成时先固定策略组合,保证zh/en对应同一“策略语义”
        strategies = pick_strategies(domain)

        for lang in langs:
            if lang == "en":
                requirement = (r.get("requirement_en") or "").strip() or (r.get("requirement_zh") or "").strip()
            ...
            # 可选:用nlg_templates覆盖overview_prefix
            ...
            meta_v2 = {
                ...
                "generator": "step05_v6_diversity_quota",
            ...
            sample = {
                "sample_id": make_id("design", f"{rid}|{lang}|{','.join(meta_v2['strategy_ids'])}|{sha1_text(text)[:6]}"),
                "task_type": "design",
                "language": lang,
                "requirement": requirement,
                "repo_context": repo_context,
    ...
    n_total = len(samples)
    if n_total == 0:
        raise RuntimeError("未生成任何Design样本,请检查design_requirements.yaml或evidence抽取。")
    ...
    print(f"Design样本生成完成:total={n_total},train={len(train)},dev={len(dev)},test={len(test)},langs={langs}")
    print(f"[Knobs]DESIGN_AUTO_EXPAND={auto_expand},DESIGN_AUTO_EXPAND_N={os.environ.get('DESIGN_AUTO_EXPAND_N','60')},DESIGN_CODE_MAX_CHARS={code_max_chars},DESIGN_EVIDENCE_K={evidence_k}")
