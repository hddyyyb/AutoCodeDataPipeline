...
def _extract_key_lines(chunks: List[Dict[str, Any]], max_lines: int = 3) -> List[Tuple[str, str]]:
    """
    从证据中抽取“像判断/SQL更新”的原句，直接预填进模板，避免模型输出xxxx。
    返回[(line, chunk_id), ...]
    """
    patterns = [
    ...
    # 兜底：没有命中if/AND/UPDATE，就拿每个chunk前1-2行“像代码”的行
    for c in chunks:
        cid = str(c["chunk_id"])
        content = c.get("content", "") or ""
        for raw in content.splitlines():
...
def _generate_once(model, tokenizer, prompt: str) -> str:
    ...
    # 额外安全：如果模型仍然吐出xxxx/Lx-Ly，直接替换为非法标记让你一眼看到
...
def main():
    ...
    for idx, (q, k) in enumerate(attempts, start=1):
        ...
        if not ok1:
            print("\n=== HARD CHECK FAILED ===")
            print(msg1)
            last_output, last_chunks = output, top_chunks
            continue
    ...
    # 两轮都失败，给出可审计的“证据不足”退出信息(这里不强行过内容校验)
    print("\n=== FINAL FAILED ===")
