...
"""
AutoCodeDataPipeline Step05(v7)
场景2:需求->基于本地代码仓架构生成设计方案(多样性+代表性+英文纯净)

修复点:
- language=en时,design_output/components/apis/risk/repo_constraints等字段全部使用英文模板,不再混中文
- 若yaml/自动扩展缺少requirement_en,且中文占比高:
  - DESIGN_STRICT_EN=2(默认推荐):丢弃该英文样本
  - DESIGN_STRICT_EN=1:用English wrapper保留原文(不推荐训练,但可审计)
  - DESIGN_STRICT_EN=0:不处理
"""
...
def generate_design_output(
    ...
    # base by domain(zh/en separated)
    if domain == "stock":
        if lang == "zh":
            components += [
    ...
    # strategy injection(zh/en separated)
    ...
    # risks(zh/en separated, use points_en)
    for s in strategies:
        if lang == "zh":
    ...
    # overview + sequence(zh/en separated)
    if lang == "zh":
...
def main():
    ...
    for t in templates:
        ...
        # 如果没有requirement_en,不要把中文拼进英文里,交给strict_en策略处理
        all_reqs.append({
    ...
    for r in all_reqs:
        ...
        for lang in langs:
            if lang == "en":
                ...
                if not requirement:
                    ...
                    if strict_en == 1:
                        ...
                        # strict_en=0:直接用中文也行(不推荐)
                        requirement = (r.get("requirement_zh") or "").strip()
                        if not requirement:
                            continue

                # 如果requirement仍包含中文,按strict策略处理
            ...
            meta_v2 = {
                ...
                "generator": "step05_v7_en_clean_quota",
