...
def validate_qa_sample(s: Dict[str, Any], index_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if "evidence" in s:
        _, hit = validate_evidence_list(s["evidence"], index_map, where)
        if hit < 1:
    ...
    return {"snip_cnt": snip_cnt, "snip_hit": snip_hit, "code_chars": code_chars}
...
def stats_for_samples(samples: List[Dict[str, Any]], kind: str) -> Dict[str, Any]:
    ...
    out = {
        ...
        "evidence_snippets_avg": evidence_snippets_avg,
        ...
        # 向后兼容旧字段名(旧Step07会读evidence_avg)
        "evidence_avg": evidence_snippets_avg,
    }
    if kind == "qa":
        out["qa_type_dist"] = dict(types)
    return out
