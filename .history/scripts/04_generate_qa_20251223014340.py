...
def wrap_answer_with_evidence_and_trace(answer_core: str, evidences: List[Dict[str, Any]], trace_steps: List[str], lang: str) -> str:
    """将核心答案包装为:结论/证据引用/推理过程(用于让模型学会输出证据与trace)"""
    is_zh = (lang == "zh")
    # 证据引用: file_path:Lx-Ly(chunk_id=...)
    ev_lines = []
    for ev in (evidences or [])[:3]:
        loc = f"{ev['file_path']}:L{ev['start_line']}-{ev['end_line']}"
        ev_lines.append(f"- {loc} (chunk_id={ev['chunk_id']})")
    ev_block = "\n".join(ev_lines) if ev_lines else ("- N/A" if is_zh else "- N/A")
...
def generate_rule_qa(rule: Dict[str, Any], index_map: Dict[str, Dict[str, Any]], templates: Dict[str, Any], lang: str) -> Dict[str, Any]:
    ...
    a = wrap_answer_with_evidence_and_trace(core, [ev], [
        ...
        ("输出结论并给出证据引用" if lang == "zh" else "Output conclusion with evidence references"),
    ], lang)
...
def generate_flow_qa(flow: Dict[str, Any], index_map: Dict[str, Dict[str, Any]], templates: Dict[str, Any], lang: str) -> Dict[str, Any]:
    ...
    core = build_flow_answer(flow, index_map, templates, lang)
    a = wrap_answer_with_evidence_and_trace(core, evs, [
        ...
        ("汇总为端到端业务流程" if lang == "zh" else "Summarize end-to-end business flow"),
    ], lang)
